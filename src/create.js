...
const repoNames = ['vite-vue-box', 'umi-tailwindCss-template']

module.exports = async (projectName) => {
	let repos = await fnLoadingByOra(fetchReopLists, '正在链接你的仓库...')()
	repos = repos
		.map((item) => item.name)
		.filter((item) => repoNames.includes(item))
	...
	const res = await fnLoadingByOra(downDir, '下载项目中...')(repoList.repo, tag)
